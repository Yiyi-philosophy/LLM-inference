...
class OptLM:
    ...
    def load_hidden(self, i, j, k):
        ...
        if i==2 and j==0 and k==0:
            print("load_hidden",self.hidden[i][j][k])
        print("store hidden:", i, j, k)
    ...
    def generate(self,
        ...
        print("# Output token ids:", self.output_ids.shape)
        ...
        if debug_mode is None:
            if not overlap:
                ...
                print("self.generation_loop_normal()")
                self.generation_loop_normal()
            else:
                # Overlap I/O and compute
                print("Overlap I/O and compute")
        ...
        for i in range(0, self.num_gpu_batches - 1): # 0-2
            ...
            for j in range(self.num_layers):
                for k in range(0, self.num_gpu_batches - i - 1): # 0-3
                    print("i,j,k=",i,j,k)
                    ipdb.set_trace()
                    # if (i==1 and j==49 and k==1) or (i==0 and j==49 and k==2): 
                    #     ipdb.set_trace()
                    self.load_weight(i, j+1, k)
    ...
    def generation_loop_debug_multi_batch(self):
        ...
        for i in range(0, self.num_gpu_batches - 1): # 0-2
            ...
            for k in range(0, self.num_gpu_batches):
                self.update_attention_mask(i, k)
            for j in range(self.num_layers):
                for k in range(0, self.num_gpu_batches): # 0-3
                    print("i,j,k=",i,j,k)
                    # ipdb.set_trace()
