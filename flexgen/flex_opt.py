...
class InputEmbed:
    ...
    def init_cache_one_gpu_batch(self, cache_home, exlen):
        pass  # do nothing
...
class SelfAttention:
    ...
    def init_cache_one_gpu_batch(self, cache_home, exlen):
        ...
        cache = device.init_cache_one_gpu_batch(self.config, self.task, self.policy, exlen)
...
class OptLM:
    def __init__(self,
        ...
        self.gsize = int(policy.num_gpu_batches/2)
    ...
    def load_weight(self, i, j, k, overlap=True, loaded=[False]):
        # Handle corner cases
    ...
    def init_cache(self, j, k, exlen):
        self.layers[j].init_cache_one_gpu_batch(self.cache_home[j][k], exlen)

    def load_cache(self, i, j, k, k_fix=0, overlap=True):
        # Handle corner cases
        if i == 0:  # prefill, no cache
            return
        ...
        if overlap:
            with torch.cuda.stream(self.load_cache_stream):
                self.layers[j].load_cache(self.cache_home[j][k-k_fix], self.cache_read_buf[j][k], i)
        else:
            self.layers[j].load_cache(self.cache_home[j][k-k_fix], self.cache_read_buf[j][k], i)

    def store_cache(self, i, j, k, k_fix=0, overlap=True):
        # Handle corner cases
        if k == -1:
        ...
        if overlap:
            with torch.cuda.stream(self.store_cache_stream):
                self.layers[j].store_cache(self.cache_home[j][k-k_fix], self.cache_write_buf[j][k], i)
        else:
            self.layers[j].store_cache(self.cache_home[j][k-k_fix], self.cache_write_buf[j][k], i)
    ...
    def compute_layer(self, i, j, k, need_pop=False):
        ...
        if need_pop:
            self.layers[j].forward(self.hidden[i][j][k], self.cache_read_buf[j][k],
                ...
                self.cache_write_buf[j][k], i, self.num_gpu_batches-1)
        else:
            self.layers[j].forward(self.hidden[i][j][k], self.cache_read_buf[j][k],
                self.weight_read_buf[j], self.attention_mask[k],
        ...
        if debug_mode is None:
            if not overlap:
                ...
                if num_gpu_batches == 1:
                    ...
                    if self.execute_gen_len>2 and self.num_gpu_batches>2 and self.policy.search_order=="diagonal":
                        self.generation_loop_overlap_multi_batch_diagonal()
                    else:
    ...
    def generation_loop_overlap_multi_batch_diagonal(self):
        # Prologue /.
        li, lj, lk = 0, 0, -1
        pi, pj, pk = 0, 0, 0
        if self.gsize>2:
        ...
        while s>0:
            ...
            for k in range(self.gsize):
                if self.sidx[k] <= pi:
                    break
                self.update_attention_mask(pi, k*2)
                self.update_attention_mask(pi, k*2+1)
            while True:
                self.load_weight(pi, pj+1, pk*2)
                self.load_cache(pi, pj, pk*2+1)
                self.store_hidden(li, lj, lk*2+1)
                self.load_hidden(pi, pj, pk*2+1)
                self.compute_layer(pi, pj, pk*2)
                self.store_cache(li, lj, lk*2+1)
                self.sync()
                
                self.load_weight(pi, pj+1, pk*2+1)
                self.load_cache(ni, nj, nk*2)
                self.store_hidden(pi, pj, pk*2)
                self.load_hidden(ni, nj, nk*2)
                self.compute_layer(pi, pj, pk*2+1, pj!=nj)
                self.store_cache(pi, pj, pk*2)
                self.sync()
                #print("present",pi,pj,pk)
                #print("next",ni,nj,nk)
                li, lj, lk = pi, pj, pk
                pi, pj, pk = ni, nj, nk
                ni, nj, nk, s = self.next_idx(ni,nj,nk,s)
                if pi != li:
                    if s==0:
                        ni, nj, nk, s = self.next_idx(pi,pj,pk,s,typ="diagonal")
                    break
            timers("generate").stop()
